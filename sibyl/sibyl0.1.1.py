...
class Sibyl:
    ...
    def auto_discretize(self,num_data,method,range_min_max):
        ...
        """This function performs automatic discretization of selected feature; a method
        (bayesian blocks, scott method or fixed bin number) along the desired data range
        is sent to a special version of hist which gives cutpoints for discretization and
        returns the "categorized" version of the original data

        Example: To categorize numerical dti data dti using scott method and save results in
        cat_dti

        cat_dti = auto_discretize(dti_array,'blocks',(dti_array.min(),dti_array.max()))
        """
    ...
    def discretize(self,columns, method = 'blocks'):
        """
        This function discretizes user provided list of numerical columns using auto_discretization function above.
        If the user desires a different method and col_range for each column, this function should be called
        on a per column basis with parameters set for each particular column
